...
def send_telegram_alert(message):
    if TELEGRAM_BOT_TOKEN and TELEGRAM_CHAT_ID:
        try:
            ...
            add_log(f"Telegram Alert Failed: {e}")
        ...
        try:
            ...
            for t in state["monitored_targets"]:
                ...
                try:
                    ...
                    t["status"] = "OFFLINE (CRITICAL)"
                    if "ONLINE" in old_status:
                        send_telegram_alert(f"⚠️ ALERT: {t['name']} ({t['url']}) is DOWN!")

            add_log(f"Cycle #{state['total_executions']} executed.")
            save_state(state)
        except Exception as err: add_log(f"Auto-Healing Recovered Engine: {err}")
        time.sleep(10)
class EnterpriseHandler(http.server.SimpleHTTPRequestHandler):
    def do_GET(self):
        if self.path == '/':
            ...
            html = f"""<!DOCTYPE html><html><head><title>OmniThread OS Enterprise</title><meta http-equiv="refresh" content="5"><style>body{{font-family:sans-serif;background:#0f172a;color:#f8fafc;padding:20px;}}.card{{background:#1e293b;padding:20px;border-radius:12px;margin-bottom:20px;border:1px solid #334155;}}h1{{color:#38bdf8;}}table{{width:100%;border-collapse:collapse;}}th,td{{padding:12px;border-bottom:1px solid #334155;}}input{{padding:8px;border-radius:6px;border:1px solid #334155;background:#0f172a;color:#fff;margin-right:8px;}}button.btn-add{{background:#22c55e;color:#fff;border:none;padding:8px 16px;border-radius:6px;font-weight:bold;}}ul{{list-style-type:none;padding:10px;max-height:250px;overflow-y:auto;font-family:monospace;background:#0f172a;border-radius:6px;}}</style></head><body><div class="card"><h1>🚀 OmniThread OS Enterprise Pro</h1><p>Status: <b style="color:#22c55e;">● {state['system_status']}</b> | Auto Cycles: <b>{state['total_executions']}</b></p></div><div class="card"><h3>Add Target</h3><form method="POST" action="/add"><input type="text" name="name" placeholder="Name" required><input type="url" name="url" placeholder="https://example.com" required><button type="submit" class="btn-add">+ Add Target</button></form></div><div class="card"><h3>Monitored Services</h3><table><thead><tr><th>Name</th><th>URL</th><th>Status</th><th>Last Check</th><th>Action</th></tr></thead><tbody>{t_rows}</tbody></table></div><div class="card"><h3>Real-Time Persistent System Logs</h3><ul>{logs_html}</ul></div></body></html>"""
    ...
    def do_POST(self):
        ...
        if self.path == '/add':
            ...
            add_log(f"Deleted Target ID #{target_id}")
            save_state(state)
...
if __name__ == "__main__":
    add_log("OmniThread OS Enterprise Pro Engine Booted.")
